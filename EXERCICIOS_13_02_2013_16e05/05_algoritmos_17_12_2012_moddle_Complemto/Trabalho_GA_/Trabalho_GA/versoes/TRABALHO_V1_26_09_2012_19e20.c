...
float AcumMedia;// acumulador de media
...
AcumMedia=0; //acumulador de media
...
scanf("%d",&qdp); //int qdp;//Quantidade de presencas
   ...
   {//cont aprovados
     ...
     printf("%d \n",ca);//test
   ...
   else if(media>=mad && pmdi<pmpa){carpf++; car++;}//distribui para  x e Y
   
   
   else if(media<mad && pmdi>=pmpa){
        ...
        //VALIDACAO DE ENTRADA PARA NOTA DA PROVA DA RECUPERACAO - PONTO EXTRA !!!!
         ...
         nmedia=media+nr;
         media=nmedia;
         AcumMedia+=media;//acumulador de media
         if(media>=mad)
         {         ca++;         }
         
         else {    carpm++; car++;        }//distribui para  x e Y
...
mgf=AcumMedia/(contpg+conttg); //mgf=(somapg+somatg)/(contpg+conttg);
